...
**vCenter SCIM vs RFC 7644 `ServiceProviderConfig`**: Outgoing SCIM clients (including Authentik worker) often call `GET …/scim/v2/ServiceProviderConfig` first. Management vCenter’s VIDB SCIM base (`…/usergroup/t/CUSTOMER/scim/v2`) may return **HTTP 404** on that path while **`/Users`** and **`/Groups`** exist (**401** without bearer). Authentik then logs **`Network error communicating with remote system`** for every object sync — that message is **misleading**; see vcf-troubleshooting **Section 43**.
