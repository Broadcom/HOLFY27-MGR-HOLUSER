...
79. **Authentik OAuth2 `property_mappings` controls which OIDC claims are included**: Without the `email`, `openid`, and `profile` scope mappings in `property_mappings`, those claims are omitted from id_tokens and SCIM sync may be incomplete. Look up PKs via `GET /api/v3/propertymappings/scope/` (names: `authentik default OAuth Mapping: OpenID 'email'` / `'openid'` / `'profile'`), then `PATCH /api/v3/providers/oauth2/{pk}/` with `{"property_mappings": [<current> + <new>]}`. `Tools/authentik_vcf_integration.py` now sets these automatically on provider creation and re-runs.
