...
**SCIM base URL** for Authentik (and manual sync): `https://vc-mgmt-a.site-a.vcf.lab/usergroup/scim/v2` — **no tenant segment**. The `/t/CUSTOMER/` path is used only for the OIDC redirect URI, not for SCIM. Using the tenant-prefixed URL causes SCIM to route incorrectly and returns 404 on Users/Groups.
...
**Default (full E2E)**: OpsToken + **Fleet IAM** public suite-api — `GET/POST .../api/fleet-management/iam/vidbs`, `.../ssorealms`, `POST .../identity-providers` (OIDC + **SCIM** + **TLS chain for the IdP host** as a JSON array of **full PEM strings**, not one string per line — see pitfalls §68), `POST .../identity-providers/{id}/directories/{dir}/sync-client` with `{"generateToken":true}` to mint the **SCIM bearer token**; then Authentik SCIM provider (set `property_mappings` and `property_mappings_group` or users will not sync — see pitfall §76), `POST .../providers/scim/{pk}/sync/` (best-effort), `PUT .../ssorealms/{realm}/principals/{groupId}/roles` (`vcf_administrator` for admin groups, `vcf_viewer` for read-only groups, `SSO_REALM` scope), and `POST .../components/auth-sources` (Join SSO for vCenter + VCF Operations + VCF Automation). Authentik users/groups: defaults `prod-admin@vcf.lab` / `dev-admin@vcf.lab` and groups `prod-admins` / `dev-admins` (admin) + `prod-readonly` (viewer).
...
Optional keys: `authentik_mgmt_vc_fqdn`, `authentik_ops_fqdn`, `authentik_issuer_base`, `authentik_application_slug`, `authentik_directory_users`, `authentik_scim_group_names` (admin role groups), `authentik_scim_filter_groups` (Authentik SCIM `group_filters`; default 7 groups), `authentik_scim_viewer_groups` (viewer role groups; default `prod-readonly`), `authentik_fleet_viewer_role` (default `vcf_viewer`), `authentik_scim_user_mapping_names`, `authentik_scim_group_mapping_names`, `authentik_scim_domain`, `authentik_fleet_idp_name`, `authentik_fleet_directory_name`, `authentik_fleet_vcf_role`, `authentik_fleet_join_nsx`, `authentik_skip_fleet_iam`, `vcf_scim_bearer_token` (legacy), `authentik_skip_coredns`, `authentik_skip_vcenter`, `authentik_skip_ops_vidb`, `authentik_api_token`, `authentik_signing_key_pk`.
...
1. `POST .../providers/scim/` with `name`, `url` (`https://<mgmt-vc>/usergroup/scim/v2` — **no `/t/CUSTOMER/`**), `token`, `verify_certificates: false` (lab). Must also set `property_mappings` and `property_mappings_group` or Authentik will sync 0 users — see pitfall §76.
2. **Property mappings** (required): look up PKs via `GET .../propertymappings/provider/scim/`; set `property_mappings: [<user-mapping-pk>]` using `authentik default SCIM Mapping: User` and `property_mappings_group: [<group-mapping-pk>]` using `authentik default SCIM Mapping: Group`.
3. **Group filter** (Authentik 2026.2+): set `group_filters` (list of group UUIDs) to limit which groups — and their members — are synced. Look up group UUIDs via `GET .../core/groups/?name=<group>`.
4. Link: `PATCH .../core/applications/<slug>/` with JSON `{"backchannel_providers": [<scim_provider_pk>]}` — URL uses **application slug** (UUID pk for PATCH returned 404 in testing).
5. **Initial directory sync**: `POST .../providers/scim/{pk}/sync/` (script tries this and `.../sync/full/`); worker may still batch — UI Schedules (Play) remains a fallback.

**vCenter SCIM vs RFC 7644 `ServiceProviderConfig`**: Outgoing SCIM clients (including Authentik worker) call `GET …/scim/v2/ServiceProviderConfig` first. Management vCenter’s VIDB SCIM base (`…/usergroup/scim/v2`) returns **HTTP 404** on that path while **`/Users`** and **`/Groups`** exist (**401** without bearer). Authentik logs **`Network error communicating with remote system`** — misleading; see vcf-troubleshooting **Section 43**.
...
71. **Authentik outgoing SCIM + vCenter `ServiceProviderConfig` 404**: Authentik worker calls `GET <scim-base>/ServiceProviderConfig`; vCenter returns **404**. Logs show **`failed to get ServiceProviderConfig`** then **`Failed to sync Group … Network error communicating with remote system`** — not a proxy outage; see vcf-troubleshooting **Section 43**.
72. **Authentik `sub_mode: user_username` breaks Fleet SSO login**: Fleet IAM maps OIDC **`sub`** → VIDB **`ExternalId`** (`Tools/authentik_fleet_iam.py`). Default Authentik outgoing SCIM **externalId** pairs with **hashed** OAuth subject, not the username string — users provision via SCIM but federated login fails until subject mode matches (see vcf-troubleshooting **Section 44**; integration script aligns this automatically when `authentik_oauth_sub_mode` is unset).
73. **Authentik `encryption_key` set → JWE id_token → VIDB NPE**: If the Authentik OAuth2 provider's *Encryption Key* field is non-null, tokens are returned as **JWE (RSA-OAEP-256)**. VIDB Java runtime cannot decrypt this algorithm; `idToken` becomes null → `NullPointerException` in `accesscontrol-service.log`. Fix: `PATCH /api/v3/providers/oauth2/<pk>/` with `{"encryption_key": null}`. `Tools/authentik_vcf_integration.py` now explicitly sets this field to `None` on provider creation.
74. **Authentik `issuer_mode: global` mismatches stored VIDB issuer**: When the IdP was registered via the Fleet IAM integration script (using `per_provider` mode), VIDB stores `oiu: "https://auth.vcf.lab/application/o/vcf/"` in `vidm_schema.fed_IdentityProvider.configuration`. Changing Authentik to `issuer_mode: global` causes tokens to carry `iss: "https://auth.vcf.lab/"` → `OidcTokenValidationException: Invalid issuer claim in token`. Fix: set `issuer_mode: per_provider` and restart `vc-ws1a-broker`. Integration script always uses `per_provider` on creation.
75. **Squid proxy blocks VIDB JWKS fetch**: `vc-ws1a-broker` (VIDB) fetches the Authentik JWKS URI (`https://auth.vcf.lab/application/o/vcf/jwks/`) to verify token signatures. If `vcf.lab` is not in the Squid allowlist, this returns **HTTP 403** → `HttpProxyConnectException` in logs → all SSO logins silently fail. Fix: add `vcf.lab` to the Squid ACL and reload (`squid -k reconfigure`). Restart `vc-ws1a-broker` to flush cached failures.
76. **Authentik SCIM provider empty `property_mappings` syncs 0 users**: Authentik reports a successful SCIM sync and updates `last_successful_sync`, but the `scim_users` count stays 0. Cause: `property_mappings` (user) and/or `property_mappings_group` (group) are empty lists `[]`. Fix: look up the PKs of `authentik default SCIM Mapping: User` and `authentik default SCIM Mapping: Group` via `GET /api/v3/propertymappings/provider/scim/` and PATCH them onto the SCIM provider. `Tools/authentik_vcf_integration.py` now handles this automatically.
77. **vCenter VIDB SCIM URL has no tenant segment**: The correct Authentik SCIM provider URL is `https://vc-mgmt-a.site-a.vcf.lab/usergroup/scim/v2`. Using `/usergroup/t/CUSTOMER/scim/v2` (the OAuth redirect URI path prefix) routes incorrectly and causes 404s on Users/Groups. The redirect URI **does** use the tenant segment; the SCIM base URL does **not**.
78. **Fleet IAM VCF role names**: Admin role is `vcf_administrator`, viewer (read-only) role is `vcf_viewer`. Both use `roleScope: {"resources": [], "scopeType": "SSO_REALM"}`. Assign via `PUT /suite-api/api/fleet-management/iam/ssorealms/{realm}/principals/{groupId}/roles` with body `{"vcfRoleAssignments": [{"roleName": "<role>", "roleScope": {...}}]}`.
79. **Authentik OAuth2 `property_mappings` controls which OIDC claims are included**: Without the `email`, `openid`, and `profile` scope mappings in `property_mappings`, those claims are omitted from id_tokens and SCIM sync may be incomplete. Look up PKs via `GET /api/v3/propertymappings/provider/scope/` (names: `authentik default OAuth Mapping: OpenID 'email'` / `'openid'` / `'profile'`), then `PATCH /api/v3/providers/oauth2/{pk}/` with `{"property_mappings": [<current> + <new>]}`. `Tools/authentik_vcf_integration.py` now sets these automatically on provider creation and re-runs.
80. **Fetching all eligible SSO components**: The endpoint `/suite-api/api/fleet-management/iam/components` only returns vCenters. To discover and configure all eligible components (including NSX managers), use `POST /suite-api/api/fleet-management/iam/components/auth-sources/query` with an empty body `{}`, which returns `iamComponentAuthSources` containing both vCenters (`componentType: VCENTER`) and NSX managers (`componentType: NSX_MANAGER`), and shows their `status` (`CONFIGURED` vs `NOT_CONFIGURED`).
81. **Stale Fleet LCM system-shutdown Argo Workflows cause VCFA HTTP 500 post-startup**: Each Fleet LCM shutdown creates an Argo Workflow (`system-shutdown-{taskid}`) in `vmsp-platform`. These persist across reboots. When `VCFfinal.py` uncordons the node, the Argo controller immediately resumes the Running workflow which re-cordons the node and scales all `prelude` Deployments to 0, causing VCFA to return HTTP 500. Up to 30+ stale workflows can accumulate, all queued on the `lock-vmsp-platform` Mutex. Fix: before any uncordon, delete all `system-shutdown-*` workflows: `kubectl get workflow -n vmsp-platform --no-headers | grep system-shutdown | awk '{print $1}' | xargs kubectl delete workflow -n vmsp-platform --grace-period=0`. `VCFfinal.py` v6.2+ does this automatically as Step 0 of Task 4b.
82. **`sudo -S kubectl` fails on VCF 9.1 VCFA node**: `kubectl` is only on root's PATH via login shell. `echo pass | sudo -S kubectl ...` returns "command not found". Always use `echo pass | sudo -S -i bash -c 'kubectl ...'` or specify `--kubeconfig=/etc/kubernetes/admin.conf` explicitly.
83. **VSP cert-manager certificates expire within lab lifecycle**: Both cluster CAs (`vcf-cluster-ca`, `vcf-external-cluster-ca-cert` in `vmsp-platform`) were created with `spec.duration: 27740h` (~3.16y). All cert expiry dates are frozen at template build time. Labs deployed 10–100×/month accumulate expired cert-manager Certificates (e.g. `tls-for-opensearch` expires ~1y after template build). Fix: run `vsp_cert_renewer.py --cluster vsp` (Phase 3.0 extends CAs to 87600h/10y; Phase 3.1 renews leaf certs to 43830h/5y). Leaf cert duration must be renewed after CA extension — leaf validity is capped by CA remaining TTL. See vcf-troubleshooting Section 51.
