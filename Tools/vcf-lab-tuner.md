...
| ------------ | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `vsp`        | VSP fleet cluster, CP VIP `10.1.1.142` (site B `10.2.1.142`), nodes `vsp-01a-*`                     | direct SSH from manager (`vmware-system-user` + `sudo -S -i`)             | `vmsp-platform`, `vcf-fleet-lcm`, `vcf-sddc-lcm`, `vidb-external`, `vodap`, `ops-logs`, `salt`, `salt-raas`, `telemetry`, `vcf-fleet-depot`, `vmsp-metrics-store` |
| `vcfa`       | VCF Automation appliance, probe `10.1.1.71-74` (live `.73`), VIPs `.69`/`.70`/`.72`                 | direct SSH from manager (`vmware-system-user` + `sudo -S -i`)             | `vmsp-platform`, `vmsp-policies`, `prelude`                                                                                                                       |
...
CLUSTERS = {
    ...
    "vcfa": {
        ...
        "candidate_ips": ["10.1.1.72", "10.1.1.73", "10.1.1.71", "10.1.1.74"],
        "transport": "direct",
        "sudo": "login",
    ...
    "ssp": {
        "label": "SSP",
        "user": "sysadmin",
        "transport": "ssp_direct",                   # SSPTransport -> auto-wraps CAPI vs Workload commands
        "sudo": "plain",
        "cp_vips": ["10.1.0.10"],
        "owned_vips": ["10.1.0.10", "10.1.0.21"],
        "worker_fqdn": "ssp-i.site-a.vcf.lab",
        "discover_octets": (10, 11, 22, 29, 31, 23, 24, 25, 26, 27, 28),
        "endpoint_fqdn": "ssp.site-a.vcf.lab",
        "endpoint_vip": "10.1.0.11",
        "endpoint_path": "/ui",
        "keeper_unit": "vcf-lab-keeper-ssp",
        "sections": ["cp", "nodes", "pods", "gateway", "deployments", "certs",
                     "endpoint", "proxy", "kubeadm", "postgres", "password", "sizing", "footprint"],
    },
}
```

**Unreachable is not an error.** Every cluster is probed first; a lab without an optional cluster (e.g. no SSP or no Supervisor) must skip that
...
sequenceDiagram
    ...
    participant SSP as SSP (ssp-i 10.1.0.10)
    ...
    MGR->>SSP: ssh sysadmin + sudo (ssp_direct / _wrap_ssp_cmd)
...
| SSP node      | `sysadmin` via direct SSH, `sudo="plain"`. Commands are wrapped by `_wrap_ssp_cmd` to automatically route CAPI management commands vs workload cluster commands (extracting `secret/ssp-kubeconfig` to `/tmp/.ssp-wlkc` with a 300s TTL cache)                                                             | `ssp_direct` (`SSPTransport`)   |
...
```text
vcf-lab-tuner.py --cluster {vsp|vcfa|supervisor|ssp|all} [--mode MODE] [options]
...
OPTIONS
    --cluster <name>       vsp | vcfa | supervisor | ssp | all   (required)
    ...
    --purge-legacy-keepers With --remove-keeper: also purge legacy_keeper_units
    --threshold-days <N>   Cert renewal threshold (default 60; confighol passes 1820)
    --target-domain <DOM>  Upstream domain filter for content library trust sync
...
| `--section`                                               | one section name                                                                                                                                           | Errors if the section does not exist for that cluster, rather than silently doing nothing                                                                                                                           |
| `--host <IP>`                                             | none                                                                                                                                                       | Skips discovery. Cannot be combined with `--cluster all`                                                                                                                                                            |
| `--dry-run`                                               | off                                                                                                                                                        | Valid with `tune`, `remediate`, `rollback`, `reset-defaults`. Structurally cannot reach the transport                                                                                                               |
| `--aggressive`                                            | off                                                                                                                                                        | Unthresholded/uncapped pod sweep (legacy `supervisor_stabilizer` behaviour)                                                                                                                                         |
| `--install-keeper`                                        | off                                                                                                                                                        | Emits + enables the 60s on-node keeper (`vcf-lab-keeper` on VSP, `vcf-lab-keeper-vcfa` on VCFA, `vcf-lab-keeper-ssp` on SSP). **Refuses if a legacy keeper is active**                                             |
| `--remove-keeper`                                         | off                                                                                                                                                        | Disables + removes it. Needs `--mode tune` or `remediate`. Mutually exclusive with `--install-keeper`                                                                                                              |
| `--purge-legacy-keepers`                                  | off                                                                                                                                                        | With `--remove-keeper`. Also stops/deletes every unit named in that cluster's `legacy_keeper_units`                                                                                                                   |
| `--threshold-days <N>`                                    | `60`                                                                                                                                                       | Cert threshold. `confighol` passes `1820` to pre-provision to 5 years                                                                                                                                               |
| `--cp-machine-type <TYPE>`                                | VSP: `cp.small/medium/large`, `management.small/medium/large/xlarge`; SSP: `standard`, `licensing`, `4/8`                                                  | `sizing` only. Requires `--cluster {vsp,ssp} --section sizing --mode remediate`                                                                                                                                     |
| `--worker-machine-type <TYPE>`                            | VSP: same choices; SSP: `standard`, `advanced`, `copilot`, `lab-reduced`, `12/56`, `16/64`                                                                 | `sizing` only                                                                                                                                                                                                       |
...
| `--no-auto-fix-autoscaler`                                | off                                                                                                                                                        | `sizing` only. Don't patch `MachineDeployment.spec.replicas` directly when the autoscaler is stuck                                                                                                                  |
| `--resize-timeout <MIN>` / `--scale-timeout <MIN>`        | `60` / `60`                                                                                                                                                | `sizing` only. CP/worker rollout wait, and replica-bounds propagation+drain wait                                                                                                                                    |
| `--poll-interval <SEC>`                                   | `5`                                                                                                                                                        | `sizing` & convergence loops polling cadence (down from 20s for rapid early exit)                                                                                                                                   |
| `--cpu-warn-pct <PCT>`                                    | `80`                                                                                                                                                       | `sizing` only. Node-utilization hot threshold (`kubectl top nodes`)                                                                                                                                                 |
| `--pin-autoscaler` / `--unpin-autoscaler`                 | off                                                                                                                                                        | `footprint` only. Durable `ReleaseTemplate.spec.helm.values.replicaCount` pin — a **different** lever from `sizing`'s `--autoscaler`                                                                                |
| `--storm-disable-le`                                      | off                                                                                                                                                        | `storm` only. OPT-IN: disables leader-election on 3 `replicas==1` vksm control services. EXPERIMENTAL                                                                                                               |
| `--storm-logging`                                         | off                                                                                                                                                        | `storm` only. OPT-IN, **DISRUPTIVE**: flips `tenant-manager-logback` DEBUG/TRACE→INFO and restarts `tenant-manager` cell                                                                                              |
| `--revert`                                                | off                                                                                                                                                        | `cp` only. Restores newest `.bak.<epoch>` backup for KCM/scheduler/etcd/kube-vip manifests                                                                                                                          |
| `--kubelet-reload`                                        | off                                                                                                                                                        | `cp` only. OPT-IN, **DISRUPTIVE**: restarts kubelet if a manifest edit ran this pass                                                                                                                                |
| `--fix-sds-sni`                                           | off                                                                                                                                                        | VCFA: fixes Envoy Gateway SDS SAN NACK errors via `platform-trust` ConfigMap synchronization (KB 439264, KB 424402)                                                                                                |
| `--cpu-tune` / `--rollback-cpu-tune`                      | off                                                                                                                                                        | VCFA: tunes Prometheus/FluentBit intervals and replica counts to reduce idle appliance load; rollback restores defaults                                                                                             |
| `--recover-gateway-503`                                   | off                                                                                                                                                        | VCFA: executes SDS fix and rolls all gateway-adjacent pods                                                                                                                                                         |
| `--export-html [PATH]`                                    | off                                                                                                                                                        | Exports standalone interactive HTML resource analysis dashboard to PATH (default: `./vlt-<cluster>-resource-report.html`)                                                                                           |
| `--site <SITE>`                                           | `a` \| `b` (default `a`)                                                                                                                                   | Retargets dynamic subnets (`10.1.x.x` vs `10.2.x.x`) and FQDNs for Site-B labs                                                                                                                                      |
| `--target-domain <DOMAIN>`                                | none                                                                                                                                                       | Supervisor `contentlib` only: upstream domain filter for content library trust sync                                                                                                                                 |
| `--no-color`                                              | auto-off when not a TTY                                                                                                                                    | Also honours `NO_COLOR` environment variable                                                                                                                                                                        |
| `-v, --verbose`                                           | off                                                                                                                                                        | Raw command output, per-item detail, per-pod role/node                                                                                                                                                              |
| `-j, --json`                                              | off                                                                                                                                                        | Machine-readable JSON document on stdout, human output suppressed                                                                                                                                                   |
...
| ------------- | ----- | ------ | ------------ | ----- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `cp`          | ✓     | ✓      |              | ✓     | `tune`, `remediate` — KCM/scheduler lease + etcd CPU tuning + kube-vip numeric lease guard; version-gated on 9.1.1+                           |
| `nodes`       | ✓     | ✓      | ✓            | ✓     | `remediate` — Ready status, uncordon (respecting autoscaler taints), capacity table, 3-column display, right-sizing analysis (`--export-html`) |
| `pods`        | ✓     | ✓      | ✓            | ✓     | `remediate` — Damped pod sweep (restartCount ≥ 5, capped at 15), phase-filtered on Supervisor (`Failed`), terminal Job cleanup              |
| `vcf`         | ✓     |        |              |       | `remediate` — Dynamic component discovery (`components.api.vmsp.vmware.com`), operational status + workload replicas                         |
| `postgres`    | ✓     | ✓      |              | ✓     | `remediate` — Spilo/Patroni pgdata (`0700`) permission fix across all namespaces (`salt-raas`, `vidb-external`, `vcf-*-lcm`, `prelude`)     |
| `redis`       | ✓     |        |              |       | `remediate` — Dynamic redis readiness + empty endpoint cert-timing race                                                                      |
| `salt`        | ✓     |        |              |       | `remediate` — Gated Salt-master restart on log error signatures                                                                              |
| `certs`       | ✓     | ✓      | ✓            | ✓     | `tune`, `remediate` — Native cert-manager leaf, backing secret x509 check, SeaweedFS mTLS freshness, and spherelet renewal                   |
| `argo`        | ✓     | ✓      |              |       | `remediate` — Stale `system-shutdown` workflows + power-off marker cleanup                                                                    |
| `kyverno`     | ✓     |        |              |       | `remediate` — UpdateRequest backlog + controllers; failurePolicy Ignore (gated to 9.1.0; native 3.8+ on 9.1.1+)                              |
| `vodap`       | ✓     |        |              |       | `remediate` — Served vs stored cert comparison + fluentd buffer purge                                                                        |
| `proxy`       | ✓     |        |              | ✓     | `tune`, `remediate` — Canonical proxy config from lsfunctions; multi-node `write_on_node` peer execution; Ubuntu/CRI-O proxy repair on SSP    |
| `kubeadm`     | ✓     | ✓      |              | ✓     | `tune`, `remediate` — Native 5-year kubeadm and kubelet cert renewal across all control plane nodes                                          |
| `password`    | ✓     |        |              | ✓     | `tune`, `remediate` — `chage -M 999` and last-change date correction                                                                         |
| `sizing`      | ✓     |        |              | ✓     | `remediate` — VSP (`cp.*`, `management.*`) and SSP (`standard`, `licensing`, `advanced`, `copilot`, `lab-reduced`) machine type / scaling    |
| `footprint`   | ✓     |        |              | ✓     | `remediate` — Lab density reduction: right-size requests, HA counts (>=1 ok), safe-to-evict, CAPI LE, autoscaler pin                         |
| `entropy`     | ✓     |        |              |       | `tune`, `remediate` — AMD Zen4/5 ESXi entropySources via govc, config-only, never reboots                                                   |
| `deployments` |       | ✓      |              | ✓     | `remediate` — Core/auth deployments, 0-replica prelude scale-up, CrashLoopBackOff pod backoff reset, NSXI platform UI                        |
| `gateway`     |       | ✓      |              | ✓     | `remediate` — LB VIPs + SDS SAN NACK auto-fix (`platform-trust` sync; native 9.1.1+ cleanup) + MetalLB LoadBalancer checks on SSP          |
| `endpoint`    |       | ✓      |              | ✓     | `remediate` — User endpoint testing (/automation, /login/, /ui) with 180s convergence polling & auth pod backoff recovery                    |
| `edge`        |       | ✓      |              |       | `remediate` — Runaway support-bundle jobs, RM gRPC self-dial deadlock (HTTP/2 SETTINGS frame), RabbitMQ `copy-config` init fix               |
| `etcd`        |       | ✓      |              |       | `remediate` — Fragmentation check + threshold-gated defrag at ≥30% slack                                                                     |
| `storm`       |       | ✓      |              |       | `remediate` — Probe relax (9.1.0), QoS elevation, opt-in leader election disable (gated by config.ini), CronWorkflow stagger                |
| `services`    |       |        | ✓            |       | `tune`, `remediate` — vCenter WCP services (`vapi-endpoint`, `trustmanagement`, `wcp`) via batched `vmon-cli`                               |
...
| ------------- | --------------------------- | --- | --- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cp`          | vsp, vcfa, ssp              | ✓   | ✓   | ✓   | VIP presence/pinning, `vip_preserve`, static pods via `crictl`, shadow-manifest sweep first                                                                                       |
| `nodes`       | vsp, vcfa, supervisor, ssp   | ✓   |     | ✓   | Ready status, `SchedulingDisabled`, uncordon (never an autoscaler-tainted node), capacity table, 3-column display, right-sizing analysis (`--export-html`)                       |
| `pods`        | vsp, vcfa, supervisor, ssp   | ✓   |     | ✓   | One row per namespace; sweep is damped by default, `--aggressive` to bypass, phase-filtered on Supervisor (`Failed`), terminal Job cleanup                                      |
| `vcf`         | vsp                         | ✓   |     | ✓   | Dynamic component discovery (`components.api.vmsp.vmware.com`), operational status + workload replicas                                                                           |
| `postgres`    | vsp, vcfa, ssp              | ✓   |     | ✓   | Spilo/Patroni pgdata (`0700`) permissions across **every** spilo namespace                                                                                                       |
| `redis`       | vsp                         | ✓   |     | ✓   | Dynamic redis readiness + empty-`redis-service`-endpoint cert-timing race                                                                                                         |
| `salt`        | vsp                         | ✓   |     | ✓   | Gated Salt-master restart on log error signatures (F7)                                                                                                                            |
| `certs`       | vsp, vcfa, supervisor, ssp  | ✓   | ✓   | ✓   | Native cert-manager leaf, backing secret x509 check, SeaweedFS mTLS freshness, and spherelet renewal without external subprocess delegation                                       |
| `argo`        | vsp, vcfa                   | ✓   |     | ✓   | Stale `system-shutdown` workflows + power-off marker cleanup                                                                                                                      |
| `kyverno`     | vsp                         | ✓   |     | ✓   | UpdateRequest backlog + controllers; failurePolicy Ignore (gated to 9.1.0; stock 3.8+ on 9.1.1+)                                                                                  |
| `vodap`       | vsp                         | ✓   |     | ✓   | Served vs stored cert comparison + fluentd buffer purge                                                                                                                           |
| `proxy`       | vsp, ssp                    | ✓   | ✓   | ✓   | Per-node proxy drift vs `lsfunctions`, multi-node `write_on_node` peer execution; Ubuntu/CRI-O proxy repair on SSP                                                                |
| `kubeadm`     | vsp, vcfa, ssp              | ✓   | ✓   | ✓   | Native 5-year kubeadm and kubelet cert renewal across all control plane nodes                                                                                                     |
| `password`    | vsp, ssp                    | ✓   | ✓   |     | `chage -M 999` **and** last-change date correction                                                                                                                                |
| `sizing`      | vsp, ssp                    | ✓   |     | ✓   | CP/worker machineType, replica bounds, autoscaler, node utilization for VSP and SSP profiles. Target flags fire with `--section sizing`                                           |
| `footprint`   | vsp, ssp                    | ✓   |     | ✓   | Lab density reduction: right-size requests, HA counts (>=1 ok), safe-to-evict, CAPI LE, autoscaler pin                                                                            |
| `entropy`     | vsp                         | ✓   | ✓   | ✓   | AMD Zen4/5 ESXi entropySources via govc, config-only, never reboots                                                                                                               |
| `deployments` | vcfa, ssp                   | ✓   |     | ✓   | Core/auth deployments, 0-replica prelude scale-up, CrashLoopBackOff pod backoff reset, NSXI platform UI                                                                           |
| `gateway`     | vcfa, ssp                   | ✓   |     | ✓   | LB VIPs + SDS SAN NACK auto-fix (`platform-trust` sync; native 9.1.1+ cleanup) + MetalLB LoadBalancer checks on SSP                                                              |
| `endpoint`    | vcfa, ssp                   | ✓   |     | ✓   | User endpoint testing (/automation, /login/, /ui) with 180s convergence polling & auth pod backoff recovery                                                                      |
| `edge`        | vcfa                        | ✓   |     | ✓   | Runaway support-bundle jobs, RM gRPC self-dial deadlock (HTTP/2 SETTINGS frame), RabbitMQ `copy-config` init fix                                                                   |
| `etcd`        | vcfa                        | ✓   |     | ✓   | Fragmentation check + threshold-gated defrag at ≥30% slack                                                                                                                       |
| `storm`       | vcfa                        | ✓   |     | ✓   | Probe relax (9.1.0), QoS elevation, opt-in leader election disable (gated by config.ini), CronWorkflow stagger                                                                    |
| `services`    | supervisor                  | ✓   | ✓   | ✓   | `vmon-cli` WCP services (`vapi-endpoint`, `trustmanagement`, `wcp`) via batched loop                                                                                             |
...
1. **One unit name per cluster, owned by one writer.** `vcf-lab-keeper` for VSP,
  `vcf-lab-keeper-vcfa` for VCFA, and `vcf-lab-keeper-ssp` for SSP. It must *replace* legacy keeper
  units (`vsp-fleet-depot-keeper`, `vcfa-eg-mem-keeper`, `vcfa-vmsp-kube-vip-keeper`, `vcfa-support-bundle-keeper`,
  `vcfa-prelude-le-keeper`, `ssp-reserve-keeper`), refusing to install while a legacy unit is active unless `--purge-legacy-keepers` is specified.
2. **Values come from the same constants the** `tune` **mode uses**, generated into the artifact at install
  time (`EG_MEM_LIMIT`, `LEASE_TRIPLE`).
...
- **VCF 9.1.0 Profile (`KEEPER_BODY_VSP`)**:
  ...
  - **Fluentd Buffer Volume Cleanup**: Automatically purges stale `/buffers/backup` directory on `logging-operator-fluentd-0`.
- **VCF 9.1.1+ Profile (`KEEPER_BODY_VSP_911`)**:
  - Eliminates obsolete probe, CPI, Kyverno, and Envoy patches that fight Flux CD reconciliation on 9.1.1+.
...
- **VCF 9.1.0 Profile (`KEEPER_BODY_VCFA`)**:
  ...
  - **8. Microservice Leader Election Hardening (Opt-In Gated)**:
    ...
    - **Opt-in Gating**: Sets `ENABLE_LEADER_ELECTION=false` environment variables on `policy-engine-server`, `policy-insights-server`, `cluster-service-server`, and `cluster-object-service-server` **only if** `disable_leader_election = true` is explicitly enabled in `/tmp/config.ini` [VCFFINAL]. By default, stock chart values are preserved.
  ...
  - **12. ReleaseTemplate FluxCD Drift Detection Disabling**: Sets `spec.helm.driftDetection.mode=disabled` across managed ReleaseTemplates (`ndc-*`, `vmsp-identity-*`, `vmsp-backup-*`, `cluster-api-installer-*`, `kyverno-1.*`).
- **VCF 9.1.1+ Profile (`KEEPER_BODY_VCFA_911`)**:
  - Dedicated 9.1.1+ keeper profile aligning strictly with VMware stock 9.1.1+ architecture.
  - Completely eliminates obsolete 9.1.0 leader election stripping, ReleaseTemplate driftDetection disablement, vsphere-cpi args, and Kyverno webhook mutations that fight Flux CD and corrupt HelmRelease reconciliation (e.g. `vksm-stack`).
  - Preserves essential lab maintenance: Envoy Gateway 4Gi memory limit, runaway support-bundle job cleanup, stale Argo system-shutdown workflow purges, 0-replica prelude scale-up, CrashLoopBackOff pod recovery, and fluentd buffer volume cleanup.
...
flowchart TD
    ...
    RUN --> TR["Transport adapters (SshMuxManager)"]
    TR --> A1["DirectTransport (vsp, vcfa)"]
    TR --> A2["SSPTransport (ssp_direct / _wrap_ssp_cmd)"]
    TR --> A3["VCenterHopTransport (vCenter -> Supervisor SCP)"]
    TR --> A4["VCenterTransport & ESXi direct"]
    RUN --> SNAP["Snapshot / Rollback / Reset Engine"]
...
- **OpenSSH Connection Multiplexing (`SshMuxManager`)**: Manages process-isolated master sockets in `/tmp/.vlt-ssh-<pid>` (`0700` permissions) with `ControlPersist=300s`, reducing SSH handshake overhead by ~80%.
- **`DirectTransport` (VSP & VCFA)**: Direct SSH connection using base64-wrapped single-quoted bash scripts over `sudo -S -i`.
- **`SSPTransport` (Security Services Platform / `ssp-i`)**: Direct SSH to `sysadmin@ssp-i` (`sudo="plain"`). Automatically wraps commands via `_wrap_ssp_cmd()` to direct CAPI management commands to the local kubeconfig (namespace `ssp`) and workload commands to `/tmp/.ssp-wlkc` extracted from `secret/ssp-kubeconfig` with a 300s TTL cache.
- **`VCenterTransport` & `VCenterHopTransport` (vCenter & Supervisor)**: Executes on vCenter as `root` and hops to the isolated Supervisor Control Plane using passwords securely decrypted on-the-fly via `decryptK8Pwd.py` into PID-isolated `0600` temp files.
...
| `auto-health.py`                 | Deprecated (superseded by vcf-lab-tuner.py `--cluster vcfa`)                                                                                                                                                                                                                                                                                                        | `--cluster vcfa --mode report`                                                                      |
...
| `vsp-health-monitor.py`          | Deprecated (superseded by vcf-lab-tuner.py + on-node drift keeper)                                                                                                                                                                                                                                                                                                  | `--mode remediate --install-keeper`                                                                 |
| `vsp-scale-down.py`              | Deprecated (superseded in v1.2.0 by `sizing` section)                                                                                                                                                                                                                                                                                                              | `--cluster {vsp,ssp} --mode remediate --section sizing`                                             |
| `vsp_cert_renewer.py`            | Deprecated (superseded in v2.0.0 by native certificate renewal engine)                                                                                                                                                                                                                                                                                              | `--section certs` / `--section kubeadm` (all clusters)                                              |
| `vcfa-stabilizer.sh`             | Deprecated (100% functional parity reached in v1.4.0)                                                                                                                                                                                                                                                                                                               | `--cluster vcfa --mode remediate`                                                                   |
| `vsp-stabilizer.sh`              | Deprecated (100% functional parity reached in v1.7.0)                                                                                                                                                                                                                                                                                                               | `--cluster vsp --mode remediate --install-keeper`                                                   |
| `supervisor_stabilizer.py`       | Deprecated (services, contentlib, webhooks, nodes, pods, native spherelet renewal all ported)                                                                                                                                                                                                                                                                       | `--cluster supervisor --mode remediate`                                                             |
...
**3.9.7 — 2026-09-23** — `vcf-lab-tuner.md` v3.9.7 / `vcf-lab-tuner.py` **v2.5.8** / **v2.5.9**.
...
- **ValueFrom Collision Prevention (`_storm_vcfa_env_le_false`, `KEEPER_BODY_VCFA`)**:
  - *WHAT*: Even when `disable_leader_election` is explicitly enabled, `_storm_vcfa_env_le_false` checks for `valueFrom` references before applying env vars.
  - *WHY*: Prevents generating invalid Kubernetes manifests that contain both `value` and `valueFrom` for the same environment variable name.
- **Dedicated VCFA VCF 9.1.1+ Drift Keeper (`KEEPER_BODY_VCFA_911`)**:
  - *WHAT*: Added dedicated `KEEPER_BODY_VCFA_911` drift keeper for VCFA clusters on VCF 9.1.1+. Eliminates obsolete 9.1.0 leader election stripping, ReleaseTemplate driftDetection disablement, vsphere-cpi args, and Kyverno webhook mutations.
  - *WHY*: On VCF 9.1.1+, upstream charts natively manage webhook failure policies, CPI args, and leader election. Disabling driftDetection or stripping args caused persistent reconciliation fights with Flux CD.
- **Resilient SSP Workload Kubeconfig Caching (`_wrap_ssp_cmd`)**:
  - *WHAT*: Added a 300s TTL cache check to the `_wrap_ssp_cmd` preamble on `ssp-i`. Automatically refreshes `/tmp/.ssp-wlkc` from `secret/ssp-kubeconfig` when the file is missing, empty, or older than 5 minutes.
  - *WHY*: Guarantees reliable workload cluster command execution even if the cached kubeconfig expires or the SSP workload cluster is re-installed mid-run.
...
### `vsp_cert_renewer.py` → Ported natively (`--section certs` / `--section kubeadm`)

**100% Ported Natively (v2.0.0)**. `vcf-lab-tuner.py` does not delegate to `vsp_cert_renewer.py`. Its entire renewal lifecycle runs internally via Python native routines (`_renew_kubeadm_certs`, `_renew_kubelet_serving_certs`, `_renew_ca_certs`, `_renew_certmanager_leaf_certs`, `_renew_antrea_certs`, `_renew_supervisor_storage_quota_certs`, `_renew_supervisor_spherelet_certs`).
Supported across all 4 cluster types (`vsp`, `vcfa`, `supervisor`, `ssp`). `--threshold-days` and `--dry-run` are natively supported.
...
| `--apply-lease` (KCM/scheduler lease 60/40/6, etcd CPU request 2500m) | Static-manifest tuning                                                                                            | `--cluster vsp --mode remediate --section cp` | **ported**                                                                                                                 |
| `--revert-lease`                                                      | Restore from backup                                                                                               | `--section cp --mode remediate --revert`      | **ported** (also supported via `--rollback` and `--reset-defaults`)                                                       |
| `--etcd-compaction`                                                   | `auto-compaction-mode=periodic` + one-time defrag                                                                 | `--cluster vsp --section etcd` / `chk_cp`     | **ported**                                                                                                                 |
| `--kube-vip-apply`                                                    | File-rewrite of the VIP lease/renew/retry timing envs                                                             | `--cluster vsp --mode remediate --section cp` | **ported** — `_kubevip_lease_guard()`, shared across clusters                                                              |
| `--status`                                                            | Keeper + numeric lease/etcd status                                                                                | `--cluster vsp --mode report`                 | **ported**                                                                                                                 |
...
| Legacy                                                                                               | What it does                                                                                                                                                                        | Equivalent                                                | Status                                                                                                                                                             |
| ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| (no args), always-on sweeps                                                                          | Stale `system-shutdown` Argo cleanup, node uncordon, terminal Job/Workflow pod cleanup, support-bundle runaway detect+fix, post-cleanup pod count, long-running-workflow diagnostic | `--cluster vcfa --mode remediate`                         | **ported** — `chk_argo`, `chk_nodes`, `chk_edge`, `chk_pods` (damped sweeps & terminal workflow cleanup)                                                          |
| Service-tls cert freshness sweep (24 named deployments, pod-start-time vs cert-renewal correlation)  | Rollout-restarts consumers of a renewed cert                                                                                                                                        | `chk_certs` (`_fix_service_tls_consumers`)                | **ported** — checks cert update timestamp vs pod `startTime` and restarts 18+ consumer deployments + SeaweedFS pods                                               |
| Phase 1.5 step 1/6 — VIP pinning + `vcfa-vip-watchdog.service` install                               | Event-driven VIP re-pinner                                                                                                                                                          | `chk_cp`                                                  | **ported** — pins VIP to active node if unassigned                                                                                                                 |
| Phase 1.5 step 2/6 — etcd CPU request + threshold-gated defrag                                       | Resource tuning & defragmentation                                                                                                                                                   | `--section etcd`                                          | **ported** — enforces CPU requests and runs defrag when threshold is exceeded                                                                                     |
| Phase 1.5 step 3/6 — kube-vip numeric lease timing on the CP static manifest                         | VIP lease timing hardening                                                                                                                          | `chk_cp`                                                  | **ported** — `_kubevip_lease_guard()`                                                                                                                              |
| Phase 1.5 step 4/6 — apiserver/KCM/scheduler probe-timeout + LE tuning                               | Static manifest probe tolerances and LE parameters                                                                                                  | `chk_storm` / `chk_cp`                                    | **ported** (gated for 9.1.0 vs 9.1.1+)                                                                                                                             |
| Phase 1.5 step 5/6 — kyverno `--forceFailurePolicyIgnore=true` (conditional)                         | Webhook failure policy hardening                                                                                                                    | `chk_kyverno`                                             | **ported** — mutates `failurePolicy: Ignore` (9.1.0 only; 9.1.1+ accepts stock)                                                                                    |
| Phase 1.5 step 6/6 — harden `vmsp-platform` kube-vip DaemonSet + install `vcfa-vmsp-kube-vip-keeper` | VIP DaemonSet tuning & keeper install                                                                                                               | `chk_cp`, `--install-keeper`                              | **ported** — `vcf-lab-keeper-vcfa` manages drift; `--purge-legacy-keepers` cleans old units                                                                        |
| RabbitMQ `copy-config` init-container restore                                                        | Init-container config recovery                                                                                                                      | `chk_edge` (`_remediate_edge_rabbitmq`)                   | **ported**                                                                                                                                                         |
| RabbitMQ `.erlang.cookie` permission fix                                                             | Secret cookie permissions                                                                                                                           | `chk_edge` (`_remediate_edge_rabbitmq`)                   | **ported**                                                                                                                                                         |
| Provisioning-service Spring Boot deadlock fix (`JAVA_OPTS` exemplars disable)                        | JVM memory/exemplar deadlock prevention                                                                                                             | `chk_edge` (`_fix_provisioning_service_exemplars`)        | **ported**                                                                                                                                                         |
| vsphere-csi-controller CrashLoopBackOff diagnosis + `dir-cli` password reset                         | CSI controller credential recovery                                                                                                                  | `chk_edge` (`_remediate_vsphere_csi`)                     | **ported**                                                                                                                                                         |
| `--fix-sds-sni` (Envoy Gateway v1.5/Envoy v1.34 SDS SAN-without-CA NACK fix)                         | SDS SNI Secret patching                                                                                                                             | `--fix-sds-sni` / `chk_edge` (`_remediate_sds_sni`)        | **ported** (with version-aware auto-detection for 9.1.0 vs 9.1.1+)                                                                                                 |
| `--cpu-tune` / `--rollback-cpu-tune` (Prometheus/FluentbitAgent tuning)                              | CPU request optimization                                                                                                                            | `--cpu-tune` / `--rollback-cpu-tune`                      | **ported**                                                                                                                                                         |
| `--recover-gateway-503` (blanket gateway-adjacent rollout-restart)                                   | Gateway outage recovery                                                                                                                             | `--recover-gateway-503`                                   | **ported**                                                                                                                                                         |
| resource-manager self-dial gRPC deadlock unblock                                                     | gRPC channel recovery                                                                                                                               | `chk_edge` (`_remediate_rm_deadlock`)                     | **ported**                                                                                                                                                         |
| 0-replica prelude Deployments → scale to 1                                                           | Replica recovery                                                                                                                                    | `chk_deployments`                                         | **ported, improved** — safely uses `vcf.lab/original-replicas` annotation                                                                                          |
...
| Legacy                                                                                                                   | What it does                                                                                                                                                 | Equivalent                                                   | Status                                                                                                                                                              |
| ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase 0b — vCenter `vapi-endpoint`/`trustmanagement`/`wcp`                                                               | `vmon-cli` check+start                                                                                                                                       | `--cluster supervisor --mode remediate --section services`   | **ported**                                                                                                                                                          |
| Phase 2/C — webhook caBundle sync                                                                                        | Name-substring match + dynamic CA resolution                                                                                                                 | `--section webhooks`                                         | **ported and improved** — resolves CA via `cert-manager.io/inject-ca-from` annotation                                                                              |
| Phase 0 — vCenter OS/VAMI proxy config                                                                                   | Proxy drop-in synchronization                                                                                                                                | `chk_proxy` (`_remediate_vcenter_proxy`)                     | **ported**                                                                                                                                                          |
| Phase 1 — content-library trust + thumbprint refresh (`govc library.update`)                                             | Live cert retrieval, trust store injection, thumbprint update, and `govc library.sync` (with deferred sync pre-flight guard for unreachable upstream depots) | `--cluster supervisor --mode remediate --section contentlib` | **ported** (with `ensure_govc` requisite checks and deferred sync guard)                                                                                           |
| Phase 2/A — hypercrypt/kubelet cold-boot recovery (30 min wait loop)                                                     | Service startup and node readiness                                                                                                                           | `chk_nodes`, `chk_services`                                  | **ported**                                                                                                                                                          |
| Phase 2/B — SCP node proxy + containerd restart                                                                          | Proxy drop-in configuration on Supervisor CP nodes                                                                                                           | `chk_proxy` (`_remediate_supervisor_scp_proxy`)               | **ported**                                                                                                                                                          |
| Phase 2/C — storage-quota cert **regeneration** (not just caBundle sync)                                                 | Storage quota webhook cert renewal                                                                                                                          | `chk_certs` (`_renew_supervisor_storage_quota_certs`)        | **ported**                                                                                                                                                          |
| Phase 2/D — CCI/ArgoCD/Harbor blind scale-up + vSphere-specific terminal-pod sweep (`AgentUnreachable`, `Evicted`, etc.) | Declarative scale-up & damped sweep                                                                                                                         | `chk_pods` (`_sweep_bad_pods`)                               | **ported and improved** — declaratively checks `replicas == 0` and gates terminal pod sweep to `Failed` phase (eliminating false warnings on `Completed` Job pods) |
| Phase 3 — ESXi spherelet cert renewal                                                                                    | ESXi spherelet certificate probe & renewal                                                                                                                   | `chk_certs` (`_renew_supervisor_spherelet_certs`)            | **ported** — auto-starts `TSM-SSH` service via vCenter pyVmomi prior to probing/renewing spherelet certificates                                                     |
...
- **`config.ini` `disable_leader_election` Opt-In Gating (`_is_disable_leader_election_enabled`, `_storm_vcfa_env_le_false`, `do_keeper`)**: Gated `ENABLE_LEADER_ELECTION=false` environment variable injection and drift keeper re-assertion behind an explicit `/tmp/config.ini` toggle (`[VCFFINAL] disable_leader_election = true`). By default (when commented out or absent), stock Helm/Flux chart leader election configurations are preserved across all microservices and versions.
- **ValueFrom Conflict Prevention (`_storm_vcfa_env_le_false`, `KEEPER_BODY_VCFA`)**: Detects `valueFrom: configMapKeyRef` in target deployments before attempting `ENABLE_LEADER_ELECTION` injection, preventing invalid Kubernetes manifests containing duplicate `value` and `valueFrom` entries.
- **Dedicated VCFA VCF 9.1.1+ Drift Keeper (`KEEPER_BODY_VCFA_911`)**: Eliminates obsolete 9.1.0 leader election stripping, ReleaseTemplate driftDetection disablement, vsphere-cpi args, and Kyverno webhook mutations on VCF 9.1.1+ VCFA appliances, avoiding Flux CD reconciliation fights during SDDC Manager upgrades (e.g. `vksm-stack`).
- **Resilient SSP Workload Kubeconfig Caching (`_wrap_ssp_cmd`)**: Added a 300s TTL cache check to the `_wrap_ssp_cmd` preamble on `ssp-i`. Automatically refreshes `/tmp/.ssp-wlkc` from `secret/ssp-kubeconfig` when missing, empty, or older than 5 minutes.

**v2.5.7 additions (2026-09-23)**: VCFA Cluster VCF 9.1.1+ Alignment & Leader Election Interruption Prevention:

- **VCFA Version Alignment & Interruption Prevention**: Completely eliminated un-gated `kubectl set env ENABLE_LEADER_ELECTION=false` mutations. Gated VCFA CAPI & Cron Stagger (`_storm_capi_le_false`, `_storm_vcfa_cron_stagger`) on 9.1.1+ to prevent modifying ReleaseTemplates or disabling Flux driftDetection on 9.1.1+.
