...
def main(lsf=None, standalone=False, dry_run=False):
    ...
    if not dry_run:
        ...
        if mgmt_hosts:
            ...
            vcf_write(lsf, 'No management hosts configured in VCF section')
    
    #==========================================================================
    # TASK 1b: VCF Automation VM fallback shutdown
    # When Fleet Operations API is unreachable (Phase 1), VCF Automation
    # VMs must be shut down directly. VM names come from [VCFFINAL] vravms.
    #==========================================================================
            ...
            if not dry_run:
                for pattern in vra_vm_patterns:
                    ...
                    if vms:
                        ...
                        for vm in vms:
                            # Skip sddcmanager - it has its own phase (16)
    ...
    vcf_ops_logs_vms = ['opslogs-01a', 'opslogs-a', 'ops-01a']
    ...
    vcf_ops_vms = ['ops-a', 'o11n-02a', 'o11n-01a']  # VCF Operations (vrops) + Orchestrator VMs
    ...
    #==========================================================================
    # TASK 13b: Shutdown VCF Automation VMs (verification/catch-all)
    # The Fleet API in Phase 1 may claim success even when no environments
    # are found (opslcm-a responds on 443 but returns empty env list).
    # This phase ensures auto-platform-a-* is actually powered off.
    # Uses [VCFFINAL] vravms and [VCF] vcfpostedgevms as VM sources.
    # If the VM is already off (Fleet API worked), this is a no-op.
    #==========================================================================
    
    vcf_write(lsf, '='*60)
    vcf_write(lsf, 'PHASE 13b: Shutdown VCF Automation VMs')
    vcf_write(lsf, '='*60)
    update_shutdown_status(13, 'Shutdown VCF Automation VMs', dry_run)
    ...
    if lsf.config.has_option('VCF', 'vcfpostedgevms'):
        postedge_raw = lsf.config.get('VCF', 'vcfpostedgevms')
        for entry in postedge_raw.split('\n'):
            entry = entry.strip()
            if entry and not entry.startswith('#'):
                vm_pattern = entry.split(':')[0].strip()
                if vm_pattern and vm_pattern not in vra_vm_patterns:
                    vra_vm_patterns.append(vm_pattern)
    
    if not dry_run:
        if vra_vm_patterns:
            vcf_write(lsf, f'Checking {len(vra_vm_patterns)} VCF Automation VM pattern(s)...')
            auto_shutdown_count = 0
            auto_already_off = 0
            for pattern in vra_vm_patterns:
                vcf_write(lsf, f'  Searching for VMs matching: {pattern}')
                vms = lsf.get_vm_match(pattern)
                if vms:
                    ...
                    for vm in vms:
                        ...
                        if is_vm_powered_on(vm):
                            vcf_write(lsf, f'  {vm.name}: Still powered on - shutting down')
                            shutdown_vm_gracefully(lsf, vm)
                            auto_shutdown_count += 1
                            time.sleep(5)
                        else:
                    ...
                    vcf_write(lsf, f'  No VMs found matching pattern')
            vcf_write(lsf, f'VCF Automation: {auto_shutdown_count} shut down, {auto_already_off} already off')
        else:
            vcf_write(lsf, 'No VCF Automation VMs configured')
    else:
        vcf_write(lsf, f'Would verify/shutdown VCF Automation VMs: {vra_vm_patterns}')
    ...
    if lsf.config.has_option('VCF', 'vcfmgmtcluster'):
        ...
        vcf_write(lsf, 'No ESXi hosts configured in [VCF] vcfmgmtcluster')
