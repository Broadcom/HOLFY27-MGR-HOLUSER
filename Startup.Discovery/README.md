# Instructions

Place Discovery specific override scripts here that will apply to ALL Discovery deployments.

1. Copy the appropriate module(s) *.py script from the Startup folder and place in this folder
2. Update the Startup.Discovery/*.py script with your custom code

For lab specific overrides, those should be placed in your vpodrepo.
